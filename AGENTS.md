...
|------|--------|------------|---------|
| **User-configured** | Explicitly set by the creator in the agent's config (UI or API) | Immutable at runtime — the agent cannot override it | User sets `dailyMaxLossPct: 5` → engine enforces a hard 5% daily realized-loss cap (percent of equity). User sets `maxDrawdownPct: 15` → engine enforces a 15% peak-to-current equity drawdown cap. |
