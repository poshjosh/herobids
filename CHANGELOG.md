...
- **Capability-scoped account tools (C4).** `get_risk_limits` and `get_account_summary` are no longer advertised by the base skill; they are restricted to trading and risk-monitoring skills. Custom skills must explicitly declare the trading capability to use either tool, and malformed legacy revisions fail closed.
...
- Fix agent creation failing with "Request validation failed" when user has no saved AI model settings — operator `modelDefaults` now used as final fallback tier for model resolution at creation time, launch time, and bot LLM resolution
...
- Added production plans and topups
...
- **Guided Setup chat-vs-form create parity:** Agents created via the Guided Setup chat (`create_agent` tool) now go through the same create-time normalization path as the form route (`POST /agents`). Shared helpers extracted to `apps/api/src/agents/` (strategy-preset-resolver, agent-create-normalization) ensure both routes get identical: unifiedConfig enrichment (technical config defaults, venue filters, metadata), tool-policy derivation (auto-grant `manage_bot` for bot-management skills), execution-mode normalization (`test` → paper/shadow based on connections), plan enforcement (agent-count limits, resolved maxBots), skill selectability validation, runtime policy overrides from user AI settings, notification policy with `enabledAt` timestamping, and authorization mode resolution. Fixes scanner-gated hybrid agents crashing on startup with `ZodError`. See `docs/features/2026/08/07/001-fix-guided-setup-missing-agent-config/001-plan.md`.

- **Chat LLM Usage Metering:** Billing metering for all chat surfaces. Every chat LLM call in Guided Setup is now metered with per-token billing using the same rate card and meter keys as worker LLM billing. Usage is accumulated across all tool rounds into one aggregate batch per user action. Billing events use `sourceType: 'chat_llm'` with idempotency anchored on persisted trigger IDs (`userMsgId` / `actionId`). Fail-open: billing errors are logged as warnings and never block the chat response. Fresh users get billing accounts created lazily on first billable chat usage. See `docs/features/2026/08/07/003-chat-llm-usage-metering/001-plan.md`.

- **Consistent hard-cap credit-check enforcement:** All three billing enforcement methods (`computeSpendStatus`, `canSpendNow`, `reserveCharge`) now use identical exact-boundary semantics: `>=` instead of `>`. When a hard cap is set, paid work is blocked as soon as available credit reaches or goes below the cap boundary — no special allowance at the exact boundary. `null` means no cap (unlimited). `0` is a real cap (block at $0.00). The `CanSpendNowResult` interface now includes `hardCapMicrousd` for consumer visibility. Billing warnings for `hardCap=0` no longer reuse meaningless percentage thresholds. Public and technical docs updated. See `docs/features/2026/08/07/002-hard-cap-credit-check-consistency/001-plan.md`.

- **Marketplace Agent Ranking — Sort by Performance Score:** A new "Ranking" sort option in the blueprint marketplace orders agents by a computed 1–10 `performanceScore` based on realized trading performance (win rate, PnL return, time-adjusted return). Scores are computed from closed position data and persisted on the `blueprints` table via migration `0063`. The score is recomputed on agent stop/start and refreshed periodically (every 6 hours). The Ranking sort is gated to agent contexts only. Trading agent cards display a `#N` rank badge. Drawdown component deferred to Phase 2. See `docs/features/2026/08/06/006-marketplace-agent-ranking/001-plan.md`.
...
- **Guided Setup thread persistence across navigation:** Navigating away from
  ...
  user resumes their existing conversation instead of starting over. The saved
  thread is cleared when the user explicitly starts over or when an agent is
  created (via confirm action or completed thread metadata). Frontend-only
  change — the backend already persisted threads.
...
- **Hybrid USD-to-Base Size Conversion:** The hybrid evaluator path now converts LLM-facing `sizeUsd` to execution-facing `targetSize` (base units) before publishing decisions. Previously, the raw USD value was forwarded directly as `targetSize`, which could inflate position sizes by orders of magnitude. The fix introduces a pricing-identity sidecar (`HybridPricingIdentity`) on `TechnicalScanState`, a dedicated `resolveHybridTargetSize()` sizing helper using `PriceService.resolvePriceTarget()`, and wires it into the hybrid `submitDecision` callback in `agent.ts`. Fail-closed: stale prices, missing chain identity for DEX signals, and invalid sizes all block submission. See [docs/features/2026/07/17/002-hybrid-usd-to-base-size-conversion/001-plan.md](docs/features/2026/07/17/002-hybrid-usd-to-base-size-conversion/001-plan.md).
