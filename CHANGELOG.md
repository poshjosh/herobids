...
- **Per-trade stop-loss and take-profit:** Agents can now set `stopLoss` and `takeProfit` price levels on every `submit_decision` that opens or increases a position. Levels are validated against mark price at intake (reject nonsensical levels), stored in-memory and in the decisions table (metadata JSONB), and monitored continuously via a 5-second periodic loop. When triggered, the engine executes a go_flat exit with per-trade metadata (`per_trade_stop_loss.triggered` / `per_trade_take_profit.triggered`). A non-blocking reminder is appended to the sync reply when levels are missing. Portfolio-level `stopLossPct` is now hidden from agent surfaces (tools and system prompt) but the operator backstop remains active.
