...
- **Zoho Mail** for hosted human mailboxes such as `admin@herobids.com` and `jane@herobids.com`
...
alerts:
  email:
    provider: ses
    fromEmail: "alerts@herobids.com"
    replyToEmail: "admin@herobids.com"
    timeoutMs: 10000
    ses:
      region: us-east-1
      accessKeyId: ""
...
- Zoho Mail hosts inboxes like `admin@herobids.com`
- SES sends application email from the verified domain
- DNS is configured so both coexist correctly on `herobids.com`
...
## 1. Replace Resend runtime wiring with provider-based composition
...
## 3. Remove Resend code completely
...
## 4. Update config schema and operator-facing documentation
...
## 5. Document the Zoho Mail operational setup
...
- Create mailboxes or aliases such as `admin@herobids.com`
...
## 6. Validate the migration end to end
