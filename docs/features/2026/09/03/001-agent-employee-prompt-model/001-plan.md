...
### Final gap review (Step 11) — resolved
- RESOLVED (was HIGH GAP-1): The connection-readiness `agent.runtime.config_update` in `apps/api/src/routes/connections.ts` built its descriptor without `skillPresetId`; since `updateRuntimeDescriptor` does a full replace, the agent identity silently reverted to "autonomous assistant" after any readiness change. Fixed by adding a shared pure `readSkillPresetId(unifiedConfig)` helper in `packages/domain/src/agent-goal.ts`, adding `unifiedConfig` to the connections select, and threading `skillPresetId` there. The session manager now uses the same shared helper (closing the item-2 DRY MEDIUM). Added `readSkillPresetId` unit tests.
- OBSERVATION (MEDIUM, deferred — out of plan scope): The scout prompt (`apps/worker/src/scout-dispatch.ts`) still uses the old `## Your Goal` / goal-executor framing and no identity line, so scout vs judge now use different mental models. The plan scoped the redesign to `buildSystemPrompt` (judge) only. Recommend a follow-up to align the scout prompt if desired.
- OBSERVATION (deferred — plan non-goal): Existing agents whose stored `prompt` is the OLD hostile `DEFAULT_BLANK_PROMPT` string are not treated as blank by `isBlankAgentGoal`, so they render that literal text under `## Your Job`. The plan explicitly excludes migrations; new agents get the non-hostile default. A one-line data backfill (`prompt=''` where `prompt=<old default>`) would close it if wanted.
- OBSERVATION (LOW): `buildFallbackRuntimeDescriptor` + the `agentGoal` fallback `'No goal provided'` in `apps/worker/src/agent.ts` would render `No goal provided` under `## Your Job` on the degraded fallback path (not blank per `isBlankAgentGoal`). Pre-existing; consider `?? ''` for consistency.
