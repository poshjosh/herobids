...
| **4** | **Frontend / dashboard** | ⬜ Next | Depends on auth (now complete). The backend already exposes the necessary data via API. UI should be designed **agent-first** (see `docs/features/2026/06/phase-5e-frontend-dashboard/000-frontend-q-and-a.md`): Agent Overview → Activity Feed → Outcome Board → Portfolio/Exposure View. Do not lead with charts or exchange jargon. Bake agent management surfaces in from day one so the UI doesn't need a rewrite when agents ship. |
