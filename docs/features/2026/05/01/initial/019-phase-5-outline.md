...
Based on docs/features/2026/05/01/initial/003-design-decisions.md, **Phase 5: Product Surface** consists of:
...
| **6** | **Agents and autonomous tool use** | 🟨 In progress | The repo now contains the first real Step 6 slice: agent CRUD and linking, protocol schemas, agent runtime/session/message/artifact persistence, worker-side broker/session/health/reconnect components, simple agent UI, and engine intake reuse. The lifecycle and context-integrity hardening work is largely landed. The current implementation path is now split cleanly between the MVP rollout plan in `docs/features/2026/06/06/005-agent-mvp-rollout-plan/001-mvp-delivery-plan.md` and the remaining canonical Step 6 completion track in `docs/features/2026/06/06/005-agent-mvp-rollout-plan/002-canonical-step-6-completion-track.md`. |
...
- **Use the new June rollout folder for planning work** — `docs/features/2026/06/06/005-agent-mvp-rollout-plan/001-mvp-delivery-plan.md` is the canonical MVP implementation order, and `docs/features/2026/06/06/005-agent-mvp-rollout-plan/002-canonical-step-6-completion-track.md` is the canonical tracker for what still blocks Step 6 from being called fully done.
