...
## Legacy Watch Limitations During Transition

Legacy watches (records without `schemaVersion` or with `schemaVersion` below 2) continue to work for basic threshold alerts but have known limitations:

- **No purpose inference:** Legacy watches without a `purpose` field are treated as having no purpose — they are neither protective nor linked to any position. They fire threshold alerts but do not contribute to position coverage evaluation.
- **Symbol fallback matching:** Legacy watches without canonical instrument identity (`instrument.instrumentId`) rely on symbol-based fallback matching against open positions. This is broad and may conflate different instruments on different venues (e.g., BTC on Hyperliquid vs. BTC on a DEX).
- **No coverage linkage:** Legacy watches without `coverage.positionKey` cannot be matched to specific positions. The runtime treats them as unlinked monitors, not as protective coverage.
- **Conservative gating:** Because legacy watches cannot be trusted for coverage evaluation, the scout/judge gating treats any open position without a v2 protective watch as uncovered, forcing judge escalation. This is intentionally conservative to avoid silent under-protection during the transition period.

These limitations are transient. As agents create new watches with v2 structured metadata, coverage evaluation accuracy improves organically. A future migration pass may backfill legacy records where deterministic upgrade is possible.