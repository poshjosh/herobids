...
OpenAIdom now supports multiple operator locales in the web app and API-facing UI flows. The implementation needs a single contract for how page chrome, persisted user preferences, API error payloads, and activity events are localized so later features do not regress into mixed English copy or ad-hoc formatting.
