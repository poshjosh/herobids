...
| AG-W05 | Wake sources — scanner implicit with pre-filter | In Strategy tab, change Filter Trades from Off → Mixed → Scanner only | Filter Trades is a 3-way selector. Off: wake source checkboxes shown. Mixed: wake source checkboxes shown; strategy presets and technical config appear. Scanner only: wake source checkboxes hidden (scanner is the sole wake source); strategy presets and technical config appear. Scanner is never shown as a checkbox. | — | 2026-07-13: Updated — Filter Trades 3-way selector replaces pre-filter toggle + hybrid mode; all in Strategy tab |
