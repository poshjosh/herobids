...
| ID | Test Case | Steps | Expected | Status | Notes |
|----|-----------|-------|----------|--------|-------|
| AG-C01 | Generic agent surfaces omit trading presentation | Open `/agents` and an agent detail for a trading-capable fixture with execution mode, strategy, and P&L data | Generic list, summary, and detail header show lifecycle and generic capability readiness only. They show no execution mode, authorization mode, strategy, P&L, trade count, win rate, trade history, or trading-specific color treatment. | ✅ | 2026-09-21; commit: `4bce03a5`; verified against a running cross-stack with an authenticated trading-capable fixture (`t1inch`, trading + gmail connection). Generic surfaces show only lifecycle + generic capability families ("Skills: trading" / "Connections: email, trading"), no trading-specific color treatment or P&L. 2026-09-23 final visual audit: operator again confirmed no trading-specific presentation on generic UI surfaces; unrelated observations recorded as future TODOs.
| AG-C02 | Capability presentation selects a bound connection | Bind two ready trading connections to one agent; mark one as the default; open its trading capability page | The page identifies and renders the default-ready connection through generic attributes and feeds; changing the selected/default binding refreshes the displayed connection without leaking the other connection's data | ✅ | Profile-backed presentation endpoint resolves the default-ready then first-ready connection server-side; explicit connectionId must be an active binding; no usable binding returns an explicit unavailable presentation (connection:null, empty attributes/feeds) rather than guessed data. Evidence: commits `6f1d038e`/`88412893`; backend unit tests in `apps/api/src/routes/capabilities/trading-presentation.test.ts` cover the default/newer-ready selection over two ready connections (asserting the older connection's identity/data is not leaked) and both `connection:null` paths; final live two-connection browser confirmation is part of C5 certification. |
| AG-C03 | Unavailable capability presentation is explicit | Open a trading capability page with no ready bound connection, then simulate an unavailable capability read | A generic unavailable/error state is shown; no stale, guessed, or raw trading values appear. C3a may use the transitional adapter only when an existing source is available. | ✅ | 2026-09-21; commit: `4bce03a5`; verified against a running cross-stack with an authenticated trading-capable fixture that has no ready bound connection. Generic unavailable/error state shown; no stale or raw trading values.
| AG-C04 | Backend semantic emphasis controls presentation | Use fixtures that return positive, negative, warning, and neutral attribute/feed emphasis | The same generic components apply their theme treatment from backend emphasis only; the frontend does not calculate P&L sign or receive CSS/color values | ✅ | Semantic emphasis is computed server-side on the API (realizedPnl sign → positive/negative on position/fill feeds; capitalAvailable false → warning on capital attribute) and the web renders only by mapping the `emphasis` field to theme tokens via `CapabilityAttributes`/`CapabilityFeeds`. Evidence: commits `6f1d038e`/`88412893`/`33e5bb58`; API unit tests cover negative→`negative` emphasis and `capitalAvailable:false`→`warning`; web `CapabilityPresentation.test.tsx` asserts emphasis→token mapping with no value interpretation. |
