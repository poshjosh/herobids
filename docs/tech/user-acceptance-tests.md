...
| A-02c | Send login link — valid email | Enter a valid email; click `Send login link` | Generic success screen shown: "Check your email" with mail icon and message; `Send another link` button visible | — | API returns `{ ok: true }` regardless of whether email exists; no email enumeration |
| A-02d | Login link sent — resend | After link sent, click `Send another link` | Resends the login link; success screen stays; no regression | — | |
| A-02e | Sign in with password expand | On login page, click `Sign in with password` link | Password field appears below email field; the link is replaced by a `Sign in` button; `Send login link` button remains visible and usable | — | 2026-07-11: New UX — password is an opt-in inline expansion |
| A-02f | Password visibility toggle | Expand password field; click the show/hide icon | Password text toggles between visible and hidden; aria-label updates accordingly | — | |
...
| A-02q | Send login link — invalid email | Enter "notanemail"; click `Send login link` | Client-side validation prevents submission or API returns 400 | — | |
...
| AG-W02 | Wake sources — trading agent layout | Create a trading agent; open Advanced Settings | AI Configuration tab: no wake source checkboxes. Trading Setup tab: shows "Receiving 3 of 3 wake sources." with 3 checkboxes all pre-checked — Watch Thresholds, Discovery Deltas, Regime Changes. Scanner is NOT listed (implicitly controlled by pre-filter toggle — on = scanner delivered, off = not). Reminders is NOT listed (always-on, forced by design). | — | |
| AG-W03 | Wake sources — reminders forced | In Trading Setup, select exactly one trading source (e.g. Watch Thresholds) | Status text reads "Receiving 1 of 3 wake sources." — counts only the visible trading sources, not the hidden forced reminder or implicit scanner. Check all 3 boxes → "Receiving 3 of 3." Uncheck all → "No sources selected. Watch Thresholds will be used by default." | — | |
...
| AG-W05 | Wake sources — scanner implicit with pre-filter | Create a trading agent; toggle pre-filter ON in Strategy tab | All three trading sources (Watch Thresholds, Discovery Deltas, Regime Changes) are pre-checked by default. Turning pre-filter ON auto-adds scanner to subscribedSources (no UI checkbox for it). Turning pre-filter OFF auto-removes scanner. Scanner is never shown as a checkbox. | — | |
