...
|----|-----------|-------|----------|--------|-------|
| A-01 | Login page renders unauthenticated | Navigate to `/` without a token | Redirected to `/login`; login card shows "Google" and "Email" tabs | ✅ | Verified: navigating to /agents while logged out redirects to /login |
...
| N-02 | Active link highlighted | Click each nav link | Current page link is visually active | ✅ | Active link highlighted on each page visit |
| N-03 | Root redirect | Navigate to `/` | Redirected to `/mission-control` | ✅ | |
| N-04 | Unknown route | Navigate to `/does-not-exist` | React Router error boundary shown (404 Not Found); does not crash | ✅ | Fixed (bug 2026-06-04-009): shows branded "Page not found" empty state with "← Back to Mission Control" CTA |
| N-05 | Page titles / headings | Visit each page | Each page has a visible `PageHeader` with title and subtitle | ✅ | Verified across Mission Control, Agents, Bots, Trading setup, Credentials, Connections, Exposure, Activity, Outcomes, Billing, Settings |
...
| ID | Test Case | Steps | Expected | Status | Notes |
|----|-----------|-------|----------|--------|-------|
| MC-01 | Summary metrics render | Open Mission Control | Shows agent-state metric cards for Active, Paused, Unhealthy, and Stopped | ✅ | Shows Active 0/1, Paused 0, Unhealthy 1, Stopped 0 |
| MC-02 | Header CTA renders | Open Mission Control | "Create agent" button shown in the page header | ✅ | |
| MC-03 | Agent overview cards | Open Mission Control with agents | One card per agent under "Your agents"; shows status, execution mode, objective, capability readiness, and actions | ✅ | Card shows crashed status, Paper mode, objective text, "Open agent" action |
| MC-04 | Recent activity feed | Open Mission Control | "Recent Activity" section on right; empty state if no events | ✅ | "No activity yet" empty state shown |
...
| I-04 | Create bot — validation error | Submit form with missing required fields | Field-level or banner error shown; form not dismissed | ✅ | "String must contain at least 1 character(s)" banner shown for empty symbol |
...
| D-02 | Unknown bot ID | Navigate to `/bots/nonexistent-id` | "Bot not found" empty state with back navigation; does not crash | ✅ | Fixed: was showing "Agent not found"; now shows "Bot not found" with "← Back to bots" button |
...
| AG-02 | Empty state | Open with no agents | "No agents yet" empty state; "Create agent" CTA | ✅ | Verified on fresh account (UAT Test User 2) — "No agents yet" with Create agent CTA shown |
| AG-03 | Create agent — happy path | Click "New agent"; fill goal, preset, and execution mode; submit | Agent detail page opens for the new agent | ✅ | Skills (Bot Management, Risk Monitoring, Trading) and Execution mode selector visible in create form |
| AG-04 | Create agent — validation | Submit with missing required fields | Error banner shown; form not dismissed | ✅ | "Review →" button is disabled when goal is empty — cannot attempt submission without filling goal; form stays open, no agent created. No error banner (validation via disabled button, not reactive banner) |
| AG-05 | Start agent | Open agent detail; click "Start" | Status transitions `stopped` → `starting` → `active`; worker picks up within ~2 s | ✅ | Status transitions to "starting"; Stop button appears immediately after clicking Start |
| AG-06 | Agent detail page renders | Click agent name | Navigates to `/agents/:id`; shows Status, Execution mode, Objective, Capabilities, Runtime Health (if active), Messages to User, Protocol Activity, and Artifacts | ✅ | All sections present; Runtime Health shown when agent active/starting |
...
| AG-09 | Stop button visibility | Open detail for active/starting/paused/unhealthy agent | "Stop" button shown | ✅ | Stop button appeared after clicking Start; stop transitions to stopped state |
...
| E-06 | No open positions | All agents have no open positions | Empty state or "No open positions" shown | ✅ | "No open positions" shown in exposure table |
...
| CN-04 | Create connection — validation | Submit with missing provider or label | Create action disabled or error shown; connection not created | ✅ | "Create" button is disabled until both provider and label are filled |
| CN-05 | Revoke active connection | Click "Revoke" on an active connection | Status updates and the revoke button disappears | ✅ | Status changes to "revoked"; Revoke button disappears |
...
| C-03 | Secret template by provider | Change the secret template in the create form | Secret fields update to match the selected template | ✅ | Bug 007 fixed: added `PROVIDER_TEMPLATES` map; Bug 015 fixed: hyperliquid now shows `apiKey`+`secret`+`walletAddress` |
| C-04 | Add credentials — validation error | Submit with a required field blank | Error shown; modal stays open | ✅ | Save button disabled when provider, label, or secret values are blank — button prevents submission; no error banner needed (same pattern as AG-04) |
...
| VA-03 | Credential dropdown in form | Open create form | Dropdown populated from existing credentials filtered to the selected venue where applicable | ✅ | Credential dropdown renders; for Jupiter venue, "Solana wallet address" field shown |
...
| VA-05 | "No credentials" label | Create account without linking a credential | "No credentials" shown in list | ✅ | "No credentials" label shown for trading accounts with no credential linked |
...
| ST-01 | Settings page renders | Navigate to `/settings` | Settings page loads without crash | ✅ | Shows Telegram notifications config section; Bug 006 fixed: Save button now disabled when chat ID matches saved value (prevents no-op saves and misleading success toast) |
...
| ID | Test Case | Steps | Expected | Status | Notes |
|----|-----------|-------|----------|--------|-------|
| ER-01 | API error message quality | Trigger a 4xx (e.g. create duplicate) | Human-readable message shown, not "HTTP 400" | ✅ | Auth shows "Invalid email or password"; registration shows "An account with this email already exists"; form validation shows readable messages |
