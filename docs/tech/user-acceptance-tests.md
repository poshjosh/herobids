...
| AG-W02 | Wake sources — trading agent layout | Create a trading agent; open Advanced Settings | AI Configuration tab: no wake source checkboxes. Trading Setup tab: shows "Receiving all wake sources. Select specific sources to filter." with 3 checkboxes — Watch Thresholds, Discovery Deltas, Regime Changes. Scanner is NOT listed (implicitly controlled by pre-filter toggle — on = scanner delivered, off = not). Reminders is NOT listed (always-on, forced by design). | — | |
| AG-W03 | Wake sources — reminders forced | In Trading Setup, select exactly one trading source (e.g. Watch Thresholds) | Status text reads "Receiving 1 of 3 wake sources." — counts only the visible trading sources, not the hidden forced reminder or implicit scanner. Check all 3 boxes → "Receiving 3 of 3." Uncheck all → "Receiving all wake sources." | — | |
...
| AG-W05 | Wake sources — scanner implicit with pre-filter | Create a trading agent; toggle pre-filter ON in Strategy tab | Watch Thresholds is pre-checked by default. Turning pre-filter ON auto-adds scanner to subscribedSources (no UI checkbox for it). Turning pre-filter OFF auto-removes scanner. Scanner is never shown as a checkbox. | — | |
