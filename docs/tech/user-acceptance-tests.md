...
| AG-E11 | Edit non-trading agent — connections visible | Open edit form for a non-trading agent | Connections section is visible in the main form, but it is optional. If active connections exist, they can be selected; if none exist, the empty state still shows the add-connection action and the form remains saveable. | — | 2026-08-01: Updated after generic providers like Gmail were added for non-trading agents |
