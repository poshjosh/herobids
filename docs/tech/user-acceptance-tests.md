...
| N-04 | Unknown route | Navigate to `/does-not-exist` | React Router error boundary shown (404 Not Found); does not crash | ✅ | Fixed (bug 2026-06-04-009): now shows branded "Page not found" empty state with "← Back to Mission Control" CTA; no longer shows React Router dev error |
...
| I-03 | Create bot — happy path | Click "Create Bot"; fill required fields; submit | Bot appears in list | ✅ | Requires at least one trading binding (venue account selector replaced with trading-binding selector — bug 2026-06-09-004 fixed) |
...
| AG-11 | Unhealthy alert banner | Open detail for agent with unhealthy session | Warning banner about missing heartbeats | ✅ | Bug found and fixed (005-stopped-agent-shows-unhealthy-banner): stopped agent was incorrectly showing unhealthy banner |
...
| C-03 | Secret template by provider | Change the secret template in the create form | Secret fields update to match the selected template | ✅ | Bug 007 fixed: added `PROVIDER_TEMPLATES` map; typing `hyperliquid` → `privateKey`, `bybit` → `apiKey`+`apiSecret`, `telegram` → `botToken`, etc. |
