...
#
# Runs `docker compose logs -f` on the server against the environment-specific
# compose files. Any extra arguments are forwarded to the logs command, allowing
# filtering by service name.
#
# Usage:
#   infra/hetzner/scripts/logs.sh [--env <staging|production>] [<server-ip>] [-- <docker-compose-args>]
#   infra/hetzner/scripts/logs.sh                          # all services, auto-detect IP
#   infra/hetzner/scripts/logs.sh --env staging            # staging environment logs
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --help|-h)
      echo "Usage: $0 [--env <staging|production>] [<server-ip>] [-- <docker-compose-logs-args>]" >&2
      echo "" >&2
      echo "Options:" >&2
      ...
      echo "  $0 --env staging           # staging environment logs" >&2
...
echo "==> Streaming ${HEROBIDS_ENV} logs from ${SERVER_IP}..."

CMD="docker compose -f /opt/herobids/docker-compose.yaml -f ${COMPOSE_OVERLAY_PATH} logs -f"
