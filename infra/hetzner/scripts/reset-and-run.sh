...
if [[ -z "$ENV_FILE" ]]; then
  # Default: look for .env.setup.prod
  ENV_FILE="$REPO_ROOT/scripts/shell/ops/.env.setup.prod"
  if [[ ! -f "$ENV_FILE" ]]; then
...
# API_BASE_URL uses localhost — quick-setup.prod.sh runs on the host OS via SSH,
...
# ─── Run quick-setup.prod.sh on the server ───────────────────────────────────

log_info "Running quick-setup.prod.sh on server (this may take a minute)..."

ssh ${SSH_OPTS} "root@${SERVER_IP}" bash -s << 'REMOTE_SETUP'
set -euo pipefail
cd /opt/herobids

echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] Running quick-setup.prod.sh..."
bash scripts/shell/ops/quick-setup.prod.sh --env-file /tmp/herobids-setup.env --yes || {
  echo "ERROR: quick-setup.prod.sh failed."
  exit 1
}
...
if [[ $exit_code -ne 0 ]]; then
  die "quick-setup.prod.sh failed on server (exit code: ${exit_code})."
fi
