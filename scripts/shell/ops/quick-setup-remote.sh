...
AI4TRADE_SKILL_TAG="trading-signals, ai4trade.ai"
...
build_flight_deal_skill_payload() {
  ...
  jq -n \
    ...
    '{
      ...
      tags: [$tag],
      changeSummary: "Seeded by prod-setup"
    }'
}
...
if [[ "$SKIP_SKILL" -eq 1 ]]; then
  ...
  ensure_flight_deal_monitoring_skill() {
    ...
    FLIGHT_DEAL_SKILL_ID="$(echo "$RESPONSE_BODY" | jq -r --arg name "$FLIGHT_DEAL_SKILL_NAME" \
      '[.skills[] | select(.name == $name) | .id][0] // empty')"
    ...
    retry_api_call POST /skills "$(build_flight_deal_skill_payload)"
    if [[ "$HTTP_STATUS" -eq 201 ]]; then
  ...
  ensure_ai4trade_skill() {
    ...
    AI4TRADE_SKILL_ID="$(echo "$RESPONSE_BODY" | jq -r --arg name "$AI4TRADE_SKILL_NAME" \
      '[.skills[] | select(.name == $name) | .id][0] // empty')"
    ...
    retry_api_call POST /skills "$(build_ai4trade_skill_payload)"
    if [[ "$HTTP_STATUS" -eq 201 ]]; then
  ...
  ensure_ict_bearish_skill() {
    ...
    ICT_BEARISH_SKILL_ID="$(echo "$RESPONSE_BODY" | jq -r --arg name "$ICT_BEARISH_SKILL_NAME" \
      '[.skills[] | select(.name == $name) | .id][0] // empty')"
    ...
    retry_api_call POST /skills "$(build_ict_bearish_skill_payload)"
    if [[ "$HTTP_STATUS" -eq 201 ]]; then
  ...
  ensure_ict_bullish_skill() {
    ...
    ICT_BULLISH_SKILL_ID="$(echo "$RESPONSE_BODY" | jq -r --arg name "$ICT_BULLISH_SKILL_NAME" \
      '[.skills[] | select(.name == $name) | .id][0] // empty')"
    ...
    retry_api_call POST /skills "$(build_ict_bullish_skill_payload)"
    if [[ "$HTTP_STATUS" -eq 201 ]]; then
...
ensure_agent() {
  ...
  retry_api_call GET /agents '?scope=mine'
  if [[ "$HTTP_STATUS" -eq 200 ]]; then
    local existing_id
  ...
  retry_api_call POST /agents "$agent_payload"
  if [[ "$HTTP_STATUS" -eq 201 ]]; then
    local created_id
  ...
  log_error "Agent creation failed for '${agent_name}' (HTTP ${HTTP_STATUS})"
