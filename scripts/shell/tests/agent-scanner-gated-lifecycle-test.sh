#!/usr/bin/env bash
# agent-scanner-gated-lifecycle-test.sh — Shell wrapper for scripts/ts/agent-scanner-gated-lifecycle-test.ts
#
# Verifies the full create→start→running lifecycle for scanner-gated agents.
# The API must produce a unifiedConfig.technical that the worker accepts at startup.
#
# Usage:
#   scripts/shell/tests/agent-scanner-gated-lifecycle-test.sh
#   scripts/shell/tests/agent-scanner-gated-lifecycle-test.sh --env /path/to/custom.env
#
# Requires:
#   - tsx or pnpm available
#   - API and DB reachable (uses docker compose exec for DB queries)
#   - Worker running (agent start requires worker)
#
# Setup:
#   cp .env.ops.dev.example .env.ops.dev
#   # fill in TEST_EMAIL and TEST_PASSWORD, then:
#   chmod +x scripts/shell/tests/agent-scanner-gated-lifecycle-test.sh
#   scripts/shell/tests/agent-scanner-gated-lifecycle-test.sh
#
# ─────────────────────────────────────────────────────────────────
...
log ""
log "=== Agent Scanner-Gated Lifecycle Test ==="
...
TS_SCRIPT="$REPO_ROOT/scripts/ts/agent-scanner-gated-lifecycle-test.ts"
