...
async function executeAssessStrategyPreset(
  ...
  // ── Broker-mediated path: port not wired (agent container context) ──
  ...
  if (ctx.publishToInbound && typeof ctx.redis.blpop === 'function') {
    ...
    try {
      await ctx.publishToInbound(AGENT_MESSAGE_TYPES.TOOL_ASSESS_STRATEGY_PRESET, requestPayload as Record<string, unknown>);
  ...
  // ── Fallback: neither port nor broker available ──
