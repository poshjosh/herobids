...
HeroBids agent messages sent to Telegram now arrive with reply mode already armed. When you reply to one of those messages, HeroBids routes your reply back to the agent that sent it.

## How It Works

1. Link your Telegram chat ID through the existing account setup flow.
2. Start an agent session.
3. HeroBids sends a session-started message that says the agent is running.
4. Every agent message also includes Telegram `ForceReply` markup.
5. Reply directly to that message in Telegram.
6. HeroBids resolves the original Telegram message back to the owning agent and delivers your reply into that agent's runtime inbox.
...
- Replies are only routed when they are direct replies to a HeroBids Telegram message.
- HeroBids verifies that the incoming Telegram `chat.id` matches the linked account owner before routing.
...
- `I couldn't find which agent that reply belongs to. The message may be too old.`
  The original Telegram message could not be mapped back to a stored HeroBids outbound message.
