...
/** Narrows a loose string (API/DOM boundary) to a canonical ReasoningLevel, or null if invalid. */
...
export function CreateAgentFlow({
  ...
  const [telegramTouched] = useState(false);
  ...
  useEffect(() => {
    ...
    if (restoredDraft) {
      ...
      dailyMaxLossPctAutoRef.current = restoredDraft.dailyLossLimitAuto;
  ...
  useEffect(() => {
    ...
    setIntent((state) => {
      ...
      const savedScoutReasoning = toReasoningLevel(savedModels.scoutReasoning);
      const savedJudgeReasoning = toReasoningLevel(savedModels.judgeReasoning);
      return {
        ...state,
        runtimePolicyOverrides: {
          ...state.runtimePolicyOverrides,
          ...(existingOverrides.scoutReasoning == null && savedScoutReasoning ? { scoutReasoning: savedScoutReasoning } : {}),
          ...(existingOverrides.judgeReasoning == null && savedJudgeReasoning ? { judgeReasoning: savedJudgeReasoning } : {}),
        },
      };
    });
      ...
      <ProviderSetupForm
        ...
        onBeforeOAuthRedirect={() => {
          ...
          saveCreateAgentOAuthDraft({
            ...
            nameIsAutoGenerated,
            dailyLossLimitAuto: dailyMaxLossPctAutoRef.current,
        ...
        <div className="create-flow-card-body" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <AgentFormBody
            ...
            onChange={(patch) => {
              ...
              setIntent((state) => {
                // AgentFormState.executionMode allows '' but the create-flow IntentState
                // only supports 'test' | 'live' — coerce an empty selection to the default.
                const executionMode: IntentState['executionMode'] =
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              setIntent((state) => {
                                ...
                                const level = toReasoningLevel(val);
                                if (level === null) {
                                  delete next.scoutReasoning;
                                } else {
                                  next.scoutReasoning = level;
                                }
                                return {
                                  ...state,
