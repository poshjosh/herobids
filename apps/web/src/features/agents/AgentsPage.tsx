          ...
          <AgentFormBody
            ...
            onChange={(patch) => {
              ...
              setIntent((state) => {
                ...
                // Scanner is meaningless without pre-filter; pre-filter implies scan interest.
                if ('technicalPreFilterEnabled' in patch) {
                  const hasScanner = next.subscribedSources.includes('scanner');
                  if (patch.technicalPreFilterEnabled && !hasScanner) {
                    next.subscribedSources = [...next.subscribedSources, 'scanner'];
                  } else if (!patch.technicalPreFilterEnabled && hasScanner) {
                    next.subscribedSources = next.subscribedSources.filter(s => s !== 'scanner');
                  }
                }
