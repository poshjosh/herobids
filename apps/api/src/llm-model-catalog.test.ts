...
describe('getAvailableProviders', () => {
  it('hides dev-only providers outside development', () => {
    setEnv('NODE_ENV', 'test');
    setEnv('LLM_API_KEY_OPENAI', 'openai-key');
    setEnv('LLM_API_KEY_OLLAMA', 'ollama-key');

    expect(getAvailableProviders({ ...BASE_CONTEXT, provider: 'ollama', baseUrl: 'http://localhost:11434/v1' })).toEqual(['openai']);
  });

  it('shows dev-only providers in development when configured', () => {
