...
import { deriveLatestVariants, getProviderCatalogEntry, type LlmCatalogDeps, type OperatorLlmCatalogContext } from './llm-model-catalog.js';
...
describe('getProviderCatalogEntry', () => {
  it('preserves isMultiProvider metadata for static providers from providers.yaml', async () => {
    const testProviders: ProvidersYaml = {
      providers: {
        deepseek: {
          catalogMode: 'static',
    ...
    // Static providers read from providersYaml directly and do not need DB.
    const deps: LlmCatalogDeps = {
      db: undefined as any,
      providersYaml: testProviders,
      context: BASE_CONTEXT,
    };

    const entry = await getProviderCatalogEntry('deepseek', deps);
    expect(entry).toBeDefined();
    expect(entry?.isMultiProvider).toBe(true);
    expect(entry?.models.map(m => m.id)).toContain('deepseek-chat');
  });
});
