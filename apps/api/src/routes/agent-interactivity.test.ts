...
describe('GET /agents/:id/export/config', () => {
  it('returns agent config with correct content-disposition header', async () => {
    const db = buildAgentDb(stubAgent);
    const redis = buildMockRedis();
    const app = Fastify();
    decorateWithAuth(app);
    ...
    const res = await app.inject({ method: 'GET', url: `/agents/${AGENT_ID}/export/config` });
    expect(res.statusCode).toBe(200);
    expect(res.headers['content-disposition']).toContain('attachment');
    expect(res.headers['content-disposition']).toContain(AGENT_ID);
  });
...
describe('GET /agents/:id/export/trades', () => {
  it('returns CSV content-type with correct headers', async () => {
    let selectCount = 0;
    const db = {
      select: vi.fn().mockImplementation(() => {
        selectCount++;
        // call 1: agent lookup, call 2: bots for agent, call 3: fills
        if (selectCount === 1) return makeChain([stubAgent]);
        return makeChain([]);
      }),
    } as unknown as Database;
    const redis = buildMockRedis();
    ...
    const res = await app.inject({ method: 'GET', url: `/agents/${AGENT_ID}/export/trades` });
    expect(res.statusCode).toBe(200);
...
describe('GET /agents/:id/export/bundle', () => {
  it('returns bundle JSON with correct content-disposition header', async () => {
    let selectCount = 0;
    const db = {
      select: vi.fn().mockImplementation(() => {
        selectCount++;
    ...
    const res = await app.inject({ method: 'GET', url: `/agents/${AGENT_ID}/export/bundle` });
    expect(res.statusCode).toBe(200);
