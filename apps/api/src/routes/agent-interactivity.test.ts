...
// ─── GET /agents/:id/trades — Traderton read-boundary re-point (c4.1) ────────

import type { TradertonClient, TradertonClientResult } from '@herobids/domain/traderton';

/**
 * Stub read client returning the given fill rows for get_agent_fills. Mirrors
 * the seam mock used in capabilities/trading.test.ts + exports.test.ts.
 */
function makeTradesReadClient(fills: unknown[]): { client: TradertonClient; invoke: ReturnType<typeof vi.fn> } {
  const invoke = vi.fn().mockImplementation((input: { toolName: string }) => {
    const payload = input.toolName === 'get_agent_fills' ? { fills } : {};
    const result: TradertonClientResult = { kind: 'success', requestId: 'r', correlationId: 'c', payload };
    return Promise.resolve(result);
  });
  return { client: { invoke } as unknown as TradertonClient, invoke };
}

const TRADE_FILL_ISO = {
  id: 'fill-1',
  orderId: 'order-1',
  venueAccountId: 'va-1',
  actorType: 'agent',
  ...
  fee: '1.00',
  feeCurrency: 'USDC',
  filledAt: new Date('2026-02-02T10:00:00.000Z').toISOString(),
  createdAt: new Date('2026-02-02T10:00:00.000Z').toISOString(),
};

describe('GET /agents/:id/trades', () => {
  it('sources fills over the boundary bound to the agent subject', async () => {
    const db = buildAgentDb(stubAgent);
    const redis = buildMockRedis();
    const { client, invoke } = makeTradesReadClient([TRADE_FILL_ISO]);
    const app = Fastify();
    decorateWithAuth(app);
    await agentInteractivityRoutes(app, db, redis, undefined, undefined, undefined, undefined, client, 10_000);

    const res = await app.inject({ method: 'GET', url: `/agents/${AGENT_ID}/trades` });
    expect(res.statusCode).toBe(200);
    const body = res.json<{ agentId: string; trades: Array<{ id: string }> }>();
    expect(body.agentId).toBe(AGENT_ID);
    expect(body.trades.map((t) => t.id)).toEqual(['fill-1']);

    const arg = invoke.mock.calls[0]![0] as { toolName: string; subject: unknown };
    expect(arg.toolName).toBe('get_agent_fills');
    expect(arg.subject).toEqual({ ownerId: TEST_USER_ID, actor: { type: 'agent', id: AGENT_ID } });
  });

  it('returns 503 precondition when the read boundary is unconfigured', async () => {
    const db = buildAgentDb(stubAgent);
    const redis = buildMockRedis();
    const app = Fastify();
    decorateWithAuth(app);
    await agentInteractivityRoutes(app, db, redis); // no read client
  ...
  it('returns 404 for an unknown agent (before touching the boundary)', async () => {
    const db = buildAgentDb(null);
    const redis = buildMockRedis();
    const { client, invoke } = makeTradesReadClient([]);
    const app = Fastify();
    decorateWithAuth(app);
    await agentInteractivityRoutes(app, db, redis, undefined, undefined, undefined, undefined, client, 10_000);
