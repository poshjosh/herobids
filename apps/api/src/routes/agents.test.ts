...
describe('agent data surface stubs (013)', () => {
  const stubPaths = [
    '/agents/agent-1/state',
    '/agents/agent-1/bots',
    '/agents/agent-1/costs',
    '/agents/agent-1/journal',
    '/agents/agent-1/trades',
  ] as const;

  it.each(stubPaths.map((url) => ['GET', url] as [string, string]))(
    '%s %s returns 200 for owned agent',
    async (_method, url) => {
      const { agentRoutes } = await import('./agents.js');

      // Build a fully-chainable mock that also resolves when awaited.
      // First select call is the ownership check; subsequent calls are data queries
      // that may chain .orderBy/.limit/.offset/.groupBy — return [].
      ...
      const db = {
        select: vi.fn().mockImplementation(() => {
          selectCallCount++;
          return selectCallCount === 1 ? makeChain([{ id: 'agent-1' }]) : makeChain([]);
        }),
      };

      const app = Fastify();
      decorateWithAuth(app);
      await agentRoutes(app, db as unknown as import('@herobids/db').Database);

      const res = await app.inject({ method: 'GET', url });
      expect(res.statusCode).toBe(200);

      // Response must contain the agentId field and correct shape key per endpoint.
      const body = res.json();
      expect(body.agentId).toBe('agent-1');
      if (url.endsWith('/state')) { expect(body).toHaveProperty('totalPnl'); expect(body).toHaveProperty('openPositionCount'); }
      if (url.endsWith('/bots')) expect(Array.isArray(body.bots)).toBe(true);
      if (url.endsWith('/costs')) expect(body).toHaveProperty('feesByCurrency');
      if (url.endsWith('/journal')) expect(Array.isArray(body.events)).toBe(true);
      if (url.endsWith('/trades')) expect(Array.isArray(body.trades)).toBe(true);
    },
  );

  it.each(stubPaths.map((url) => ['GET', url] as [string, string]))(
    '%s %s returns 404 for unknown agent',
    async (_method, url) => {
      const { agentRoutes } = await import('./agents.js');
      const { db } = buildDb({ agentRows: [] });
