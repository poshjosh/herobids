...
describe.skipIf(SKIP)('Agent interactivity functional', () => {
  ...
  describe('GET /agents/:id/export/config', () => {
    it('returns agent JSON with attachment header', async () => {
      const id = await createAgent();

      const res = await ctx.app.inject({
        method: 'GET',
        url: `/agents/${id}/export/config`,
        headers: authHeader(),
      });

      expect(res.statusCode).toBe(200);
      expect(res.headers['content-disposition']).toContain('attachment');
      expect(res.headers['content-disposition']).toContain(id);
    });

    it('returns 404 for unknown agent', async () => {
      const res = await ctx.app.inject({
        method: 'GET',
        url: '/agents/nonexistent-id/export/config',
        headers: authHeader(),
      });
      expect(res.statusCode).toBe(404);
    });
  ...
  describe('GET /agents/:id/export/trades', () => {
    it('returns CSV with correct content-type and attachment disposition', async () => {
      const id = await createAgent();

      const res = await ctx.app.inject({
        method: 'GET',
        url: `/agents/${id}/export/trades`,
        headers: authHeader(),
      });
  ...
  describe('GET /agents/:id/export/sessions', () => {
    it('returns CSV with correct headers', async () => {
      const id = await createAgent();

      const res = await ctx.app.inject({
        method: 'GET',
        url: `/agents/${id}/export/sessions`,
        headers: authHeader(),
      });
